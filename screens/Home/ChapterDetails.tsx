...
export default function ChapterList({
  ...
  const { data, isRefetching, isLoading, refetch, fetchNextPage } =
    useInfiniteQuery<QueryKey<Chapter.Request>, IError, Chapter.ListResponse>(
      [MANGA_FEED(mangaId), {
        limit: 10, translatedLanguage: ["en"], order: {
          volume: "desc",
          chapter: "desc"
        }
      } as Chapter.Request],
  ...
  return <SectionList
    ...
    renderSectionHeader={
      ({ section: { title } }) => <View padding={1} backgroundColor="white" marginBottom={2}><Heading width="100%" textAlign="center">Volume {title ?? "None"}</Heading></View>
    }
    renderItem={({ item }) => (
      <Thumbnail {...item} />
    )}
    ...
    ListFooterComponent={(totalChapters && noOfChapters && noOfChapters < totalChapters) || isLoading ? <Duplicate Component={ThumbnailSkeleton} times={4} /> : null}
    onEndReached={() => {
